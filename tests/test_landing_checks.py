...
def test_landing_checks_valid(tmp_path):
...
def test_landing_checks_missing_file():
...
def test_landing_checks_empty_json(tmp_path):
...
def test_landing_checks_unsupported_resource(tmp_path):
    p = tmp_path / "valid.json"
    p.write_text(json.dumps([{"id": 1}]))
